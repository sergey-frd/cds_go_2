...
//   Total_Slot_Price :=   TotalDict["Ow_Day_Ds"][string(ow_day_ds_key_enc)

//----------------------------------------------
func Alloc_Um_Ds(byteValues  []byte, 
    data           map[string]map[string]string,
    TotalDict      map[string]map[string]float64,
    um             S.User_Media_STC, 
    Day_Media_Cost float64,
    )  (err error) {
    ...
    tbl_name   = "Ow_Day_Ds"
    var keys []string
    for k, _ := range TotalDict[tbl_name] {
        keys = append(keys, k)
    } // for k, v
    sort.Strings(keys)
    // n := 0
    ...
    for _, k := range keys  {
        ...
        err = json.Unmarshal(byt_k, &ods.Ow_day_ds_key)
        if err != nil {
            fmt.Println("There was an error:", err)
        }
        
        //byt_v := []byte(v)
        //err = json.Unmarshal(byt_v, &ods.Total_Cost)
        //if err != nil {
        //    fmt.Println("There was an error:", err)
        //}

        nnn  += 1
        TotalDict["Sum_Ow_Ds"][k]  = Day_Media_Cost / ods.Total_Cost
        TotalDict["Paid_Ow_Ds"][k] = 0

        //if nnn < 5 { 
            //fmt.Println(nnn,"k = ", k)
        //    fmt.Println(nnn,"TotalDict[Paid_Ow_Ds][k] = ",ods.Ow_day_ds_key, TotalDict["Paid_Ow_Ds"][k]) 
        //}
    ...
    // 
    err = sum_Um_Ds(byteValues,data,TotalDict,um,);  __err_panic(err)
