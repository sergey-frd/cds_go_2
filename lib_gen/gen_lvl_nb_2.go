...
func Gen_Lvl_Nb2(byteValues  []byte, 
    ...
    for _, k := range keys  {
        ...
        if Nb.CnCtNb.CnCt != Um_nbds.Um_nbds_key.Ds_keys[0].CnCtNb.CnCt {
            continue
        }
        ...
        //fmt.Println("um =", um)
        fmt.Println("    nb Um_nbds =", Um_nbds)
        //err = Gen_Lvl_Ds2(byteValues,data,Um_nbds,um,indx+nnn-1,);  __err_panic(err)
        err = Gen_Lvl_Ds2(byteValues,data,Um_nbds,um,indx,);  __err_panic(err)
